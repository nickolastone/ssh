// Package sshder handles the creation of new SSH key pairs.
package ssh
...
func (s *SSHKeyPair) PublicKey() []byte {
	...
	return pubKeyWithMemo(ak, s.memo)
}
...
func pubKeyWithMemo(pubKey []byte, memo string) []byte {
	if memo == "" {
		u, err := user.Current()
		if err != nil {
			return pubKey
